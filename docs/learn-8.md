...
#### HTTP 超文本传输协议
常用字段：
connection: Keep-Alive  # 请求头要求使用tcp持久连接，直到服务端或客户端关闭
Content-Length      # 服务器响应数据总长
Content-Type: text/html;charset=utf-8    # 服务器告诉客户端本次响应的数据类型，发送的是网页，编码是utf-8
Accept: */*   # 客户端告诉服务器可以接收什么类型的数据
Content-Encoding: gzip   # 服务器告诉客户端响应数据使用了gzip压缩
Accept-Encoding: gzip, deflate # 客户端接受哪种压缩方式


#### 版本
1. http/1.0   每次请求都要建立3次握手的tcp开销
2. http/1.1   增加了tcp持久(长)连接,双方没有明确提出断开则一直保持tcp连接。若超时，服务端主动断开。
    具备管道传输功能，第一个请求发出后，不用等回复就可发送第二个请求。但服务器必须按照请求顺序逐一回复。存在响应的队头堵塞。
